...
var uuid = (__webpack_require__(/*! uuid */ "./node_modules/uuid/dist/commonjs-browser/index.js").v4);
var sessions = {};
...
function getSessionUser(sid) {
  var _sessions$sid;
  return (_sessions$sid = sessions[sid]) === null || _sessions$sid === void 0 ? void 0 : _sessions$sid.username;
}
...
/* harmony export */   "fetchGetMessage": () => (/* binding */ fetchGetMessage),
...
function renderMessageView() {
  var html = "\n    <div class=\"users__container\">\n\n      ".concat(renderCurrentUsers(), "\n    </div>\n    <div class=\"messages__container\">\n\n      ").concat(renderPostedMessages(), "\n    </div>\n    <div class=\"user__container\">\n      ").concat(displayUsername(), "\n      ").concat(displayLogoutBtn(), "\n      ").concat(displayMessageForm(), "\n  ");
  appEl.innerHTML = html;
}
function renderUsersAndMessages() {
  var usersContainerEl = document.querySelector(".users__container");
  var messagesContainerEl = document.querySelector(".messages__container");
  var currentUserEl = document.querySelector(".word__username");
  var userHtml = renderCurrentUsers();
  var messageHtml = renderPostedMessages();
  var usernameHtml = displayUsername();
...
function renderCurrentUsers() {
  var listHtml = Array.from(Object.keys(state.allLoginUsers)).map(function (name) {
    return "\n      <li class=\"user\">\n        <span class=\"user__name\">".concat(name, "</span>\n      </li>\n    ");
  }).join("");
  var html = "\n    <h2 class=\"users__heading\">Current Logged-in Users: </h2>\n    <ul class=\"users\">".concat(listHtml, "</ul>\n  ");
  return html;
}
function renderPostedMessages() {
  var listHtml = state.postedMessages.map(function (el) {
    var messageInfo = Object.values(el)[0];
    return "\n      <li class=\"message\">\n        <span class=\"message__sender\">".concat(messageInfo.user, ": </span>\n        <span class=\"message__text\">").concat(messageInfo.message, "</span>\n      </li>\n    ");
  }).join("");
  var html = "\n    <h2 class=\"messages__heading\">Previous Messages: </h2>\n    <ul class=\"messages\">".concat(listHtml, "</ul>\n  ");
  return html;
}
function displayUsername() {
  return "\n  <p class=\"word__username\"> You are logged in: ".concat(state.currentUser, " </p>\n  ");
}
...
module.exports = {
  ...
  renderUsersAndMessages: renderUsersAndMessages,
  renderCurrentUsers: renderCurrentUsers,
  renderPostedMessages: renderPostedMessages
};
...
var _require = __webpack_require__(/*! ./services */ "./src/services.js"),
  ...
  fetchUser = _require.fetchUser,
  fetchLogin = _require.fetchLogin,
  fetchLogout = _require.fetchLogout,
  fetchGetMessage = _require.fetchGetMessage,
  fetchPostMessage = _require.fetchPostMessage;
var view = __webpack_require__(/*! ./view */ "./src/view.js");
var sessions = __webpack_require__(/*! ./../sessions */ "./sessions.js");
var _require2 = __webpack_require__(/*! ./state */ "./src/state.js"),
  state = _require2.state,
  ERROR_MESSAGE = _require2.ERROR_MESSAGE,
  PAGES = _require2.PAGES;
var appEl = document.querySelector(".app");
var timer;
function init() {
  view.renderLoadingIndicator();
  fetchUser().then(function (_ref) {
  ...
  appEl.addEventListener("submit", function (event) {
    ...
    var sendMessageBtn = event.target.querySelector(".form__message-button");
    if (loginBtn) {
    ...
    if (logoutBtn) {
      ...
      fetchLogout().then(function () {
        ...
        var messageViewErrorEl = document.querySelector(".form__message-error");
        messageViewErrorEl.innerText = ERROR_MESSAGE[error] || ERROR_MESSAGE["default"];
      });
    }
    if (sendMessageBtn) {
      event.preventDefault();
      var messageInput = event.target.querySelector(".form__message-input");
      var message = messageInput.value;
      fetchPostMessage(message).then(function (response) {
        messageInput.value = "";
        return fetchGetMessage();
      }).then(function (_ref7) {
        var username = _ref7.username,
          previousMessages = _ref7.previousMessages;
...
function updateMessagesToState(previousMessages) {
  ...
  Object.keys(previousMessages).forEach(function (user) {
    previousMessages[user].forEach(function (_ref9) {
  ...
  state.postedMessages = state.postedMessages.sort(function (a, b) {
    return Object.keys(b)[0] - Object.keys(a)[0];
  });
}
function updateLoginSessionsToState(sessions) {
  state.allLoginUsers = {};
  Object.entries(sessions).forEach(function (_ref10) {
    var _ref11 = _slicedToArray(_ref10, 2),
      sid = _ref11[0],
      username = _ref11[1].username;
...
function refreshUsers() {
  fetchUsers().then(function (_ref12) {
    var sessions = _ref12.sessions;
    updateLoginSessionsToState(sessions);
    return fetchGetMessage();
  }).then(function (_ref13) {
    var username = _ref13.username,
      previousMessages = _ref13.previousMessages;
    updateMessagesToState(previousMessages);
